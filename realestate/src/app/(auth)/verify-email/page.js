...
import { useVerifyemailMutation } from "@/lib/services/authSlice";
...
const EmailVerifyOtp = () => {
  ...
  const { values, errors, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (values, action) => {
      setLoading(true);
      try {
        const responce = await verifyemail(values);
    ...
    <section className=" flex flex-col align-middle justify-center items-center w-full h-[100vh]  bg-violet-300">
      ...
      >
        ...
        <input
          ...
          required
        />
        {errors.otp && <div className="text-sm text-red-500">{errors.otp}</div>}
        <Link
          href={"/forgotpassword"}
